...
void Update()
{
	...
	//up
	if (catInput == 3 && cat_collide) { cat.catPos.y -= 10; }
	...
	if (cat.catPos.x + cat.catPos.w >= wall.wallPos.x){ cat.catPos.x = wall.wallPos.x - cat.catPos.w;}
	...
	if (cat.catPos.x <= wall2.wallPos.x + wall2.wallPos.w){cat.catPos.x = wall2.wallPos.x + wall2.wallPos.w;}
